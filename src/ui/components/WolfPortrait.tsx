...
export function WolfPortrait({ wolf, size = 'medium', showDetails = true }: WolfPortraitProps) {
  const sizeClasses = {
    small: 'w-16 h-16',
    medium: 'w-24 h-24',
    large: 'w-32 h-32'
  ...
  const baseColor = getColorValue(wolf.appearance.baseColor || wolf.appearance.furColor);
  const markingColor = wolf.appearance.markingColor ? getColorValue(wolf.appearance.markingColor) : undefined;
  ...
  const renderSimplePortrait = () => (
    ...
    >
      ...
      <div className="relative w-full h-full flex items-center justify-center">
        {/* Eyes */}
        <div className="absolute top-1/3 left-1/4 w-2 h-2 rounded-full border border-black"
             style={{ backgroundColor: eyeColor }}></div>
        <div className="absolute top-1/3 right-1/4 w-2 h-2 rounded-full border border-black"
             style={{ backgroundColor: eyeColor }}></div>
        ...
        {wolf.appearance.pattern !== 'solid' && markingColor && (
          <div 
            ...
            style={{
              background: wolf.appearance.pattern === 'striped' 
                ? `repeating-linear-gradient(45deg, transparent, transparent 2px, ${markingColor} 2px, ${markingColor} 4px)`
                : wolf.appearance.pattern === 'spotted' || wolf.appearance.markingType === 'spotted'
  ...
  const renderDetailedInfo = () => (
    <div className="text-xs space-y-1 mt-2">
      ...
      <div className="text-gray-300">
        {wolf.appearance.baseColor || wolf.appearance.furColor} {wolf.appearance.pattern}
      </div>
      <div className="text-gray-400">{wolf.appearance.eyeColor} eyes</div>
      {wolf.appearance.scars.length > 0 && (
        <div className="text-red-400 text-xs">
          {wolf.appearance.scars.length} scar{wolf.appearance.scars.length > 1 ? 's' : ''}
        </div>
      )}

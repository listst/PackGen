...
export class WolfGenerator {
  ...
  private readonly BIOME_NAME_POOLS = {
    forest: {
      nature: [
        'Oak', 'Pine', 'Cedar', 'Willow', 'Birch', 'Maple', 'Elm', 'Fern',
        'Moss', 'Ivy', 'Bramble', 'Sage', 'Grove', 'Shade', 'Thorn', 'Brook',
        'Stream', 'Glade', 'Hollow', 'Branch', 'Leaf', 'Root', 'Bark', 'Timber'
      ],
    ...
    mountain: {
      nature: [
        'Stone', 'Rock', 'Granite', 'Slate', 'Cliff', 'Peak', 'Summit', 'Ridge',
        'Boulder', 'Crag', 'Ledge', 'Scree', 'Alpine', 'Glacier', 'Avalanche'
      ],
      weather: ['Wind', 'Storm', 'Thunder', 'Lightning', 'Blizzard', 'Frost', 'Snow'],
      features: ['Canyon', 'Gorge', 'Pass', 'Vista', 'Echo', 'Chasm']
    },
    plains: {
      nature: [
        'Grass', 'Prairie', 'Meadow', 'Field', 'Wheat', 'Barley', 'Rye', 'Oat',
        'Clover', 'Heather', 'Sage', 'Horizon', 'Distant', 'Endless', 'Open'
      ],
    ...
    tundra: {
      nature: [
        'Snow', 'Ice', 'Frost', 'Winter', 'Polar', 'Arctic', 'Frozen', 'Crystal',
        'Lichen', 'Permafrost', 'Tundra', 'North', 'Boreal', 'Frigid'
      ],
    ...
    desert: {
      nature: [
        'Sand', 'Dune', 'Oasis', 'Cactus', 'Sage', 'Mesa', 'Butte', 'Sandstone',
        'Amber', 'Gold', 'Copper', 'Bronze', 'Rust', 'Sienna', 'Ochre'
      ],
    ...
    swamp: {
      nature: [
        'Marsh', 'Bog', 'Mud', 'Reed', 'Cattail', 'Cypress', 'Moss', 'Lily',
        'Mangrove', 'Mire', 'Fen', 'Peat', 'Algae', 'Vine', 'Creeper'
      ],
  ...
  private readonly COMMON_NAME_POOLS = {
    characteristics: [
      'Swift', 'Strong', 'Keen', 'Bold', 'Wise', 'Brave', 'Wild', 'Free',
      'Noble', 'Fierce', 'Gentle', 'Sharp', 'Quick', 'Steady', 'Bright', 'Dark'
    ],
    colors: [
      'Silver', 'Gray', 'Black', 'White', 'Brown', 'Red', 'Gold', 'Copper',
      'Russet', 'Tawny', 'Sable', 'Cream', 'Ash', 'Smoke', 'Shadow'
    ],
  };
  ...
  generateRandomPack(biome: string = 'forest'): Wolf[] {
    const packSize = random.nextInt(this.config.minPackSize, this.config.maxPackSize);
    const wolves: Wolf[] = [];
  ...
  private generateRandomName(biome: string = 'forest'): string {
    ...
    const biomePool = this.BIOME_NAME_POOLS[biome as keyof typeof this.BIOME_NAME_POOLS] || this.BIOME_NAME_POOLS.forest;
  ...
  private generateRandomStats(role: Role) {
    ...
    switch (role) {
      ...
      case 'elder':
        ...
        baseStats.strength = Math.max(3, baseStats.strength - random.nextInt(1, 2));
  ...
  private generateRandomTraits(role: Role) {
    ...
    switch (role) {
      ...
      case 'elder':
        ...
        baseTraits.fertility = Math.max(1, baseTraits.fertility - random.nextInt(2, 4));
        break;
    }
  ...
  private applyAgeDistribution(wolves: Wolf[]): void {
    ...
    const targetPups = Math.floor(wolves.length * this.config.ageDistribution.pupsPercent);

    // Adjust ages if needed (simplified approach)
    const adjustableWolves = wolves.filter(w => 
      !this.config.guaranteedRoles.includes(w.role) || w.role === 'omega'
    );
  ...
  private createMatingPairs(wolves: Wolf[]): void {
    const breedingAgeWolves = wolves.filter(w => 
      w.age >= 2 && w.age < 8 && (w.role === 'alpha' || w.role === 'beta' || w.role === 'hunter')
    );
    ...
    const maxPairs = Math.min(2, Math.floor(Math.min(males.length, females.length)));
  ...
  generateRandomBiome(): string {
    const biomes = ['forest', 'mountain', 'plains', 'tundra', 'desert', 'swamp'];
    return random.choice(biomes);
  }

  // Generate a specific wolf type for testing
  generateSpecificWolf(role: Role, name?: string, biome: string = 'forest'): Wolf {
    const wolf = this.generateWolfWithRole(role, biome);
    if (name) {
      wolf.name = name;
    }

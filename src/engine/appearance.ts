...
export class AppearanceGenerator {
  ...
  setConfig(_config: GameConfig): void {
    // Configuration can be added in future versions
  }
  ...
  generateRandomAppearance(): Appearance {
    ...
    const colorCombo = this.COLOR_COMBINATIONS[baseColor as keyof typeof this.COLOR_COMBINATIONS] 
    ...
    const appearance: Appearance = {
      ...
      furLength: random.choice(this.FUR_CHARACTERISTICS.length) as 'short' | 'medium' | 'long',
      furTexture: random.choice(this.FUR_CHARACTERISTICS.texture) as 'smooth' | 'coarse' | 'fluffy',
      bodySize: random.choice(this.FUR_CHARACTERISTICS.bodySize) as 'small' | 'medium' | 'large',
      earShape: random.choice(this.FUR_CHARACTERISTICS.earShape) as 'pointed' | 'rounded' | 'large',
      tailType: random.choice(this.FUR_CHARACTERISTICS.tailType) as 'thick' | 'bushy' | 'thin' | 'plume'
    };
  ...
  private selectEyeColor(pattern: string): string {
    ...
    let weights = Object.values(this.EYE_COLOR_GENETICS).map(e => e.weight);
  ...
  private generateScars(): string[] {
    ...
    if (random.next() < scarChance) {
      const scarTypes = [
        'ear notch', 'facial scar', 'leg scar', 'shoulder mark',
        'chest mark', 'back stripe', 'tail tip', 'paw scar',
        'neck scar', 'flank mark'
      ];
  ...
  private geneticInheritance(motherApp: Appearance, fatherApp: Appearance): Appearance {
    ...
    if (random.next() < 0.05) { // 5% mutation chance
  ...
  private inheritEyeColor(motherApp: Appearance, fatherApp: Appearance): string {
    ...
    if ((motherEye === 'green' || fatherEye === 'green') && random.next() < 0.2) {
